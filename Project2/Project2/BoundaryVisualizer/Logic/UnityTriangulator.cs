...
{
    public class UnityTriangulator
    {
        private UnityTriangulator()
        {
        }

        public static List<Triangle> earcut(List<PointF> data)
        {
            int pointer = 0;
         ...
         */
        public static List<Triangle> earcut(double[] data)
        {
         ...
         * @return List containing groups of three vertice indices in the resulting array forms a triangle.
         */
        public static List<Triangle> earcut(double[] data, int[] holeIndices, int dim)
        {
            ...
            earcutLinked(outerNode, triangles, dim, minX, minY, size, int.MinValue);
            List<Triangle> actualTriangles = new List<Triangle>();
            for (int i = 0; i < triangles.Count; i += 3)
            {
                actualTriangles.Add(new Triangle(new PointF((float)data[triangles[i] * 2], (float)data[triangles[i] * 2 + 1]), new PointF((float)data[triangles[i + 1] * 2], (float)data[triangles[i + 1] * 2 + 1]), new PointF((float)data[triangles[i + 2] * 2], (float)data[triangles[i + 2] * 2] + 1)));
            }

            return actualTriangles;
        }

        private static void earcutLinked(Node ear, List<int> triangles, int dim, double minX, double minY, double size, int pass)
        {
            if (ear == null)
                return;

            // interlink polygon nodes in z-order
            if (pass == int.MinValue && size != double.MinValue)
            ...
            while (ear.prev != ear.next)
            {
                ...
                if (size != double.MinValue ? isEarHashed(ear, minX, minY, size) : isEar(ear))
                {
        ...
        private static bool isEarHashed(Node ear, double minX, double minY, double size)
        {
            Node a = ear.prev;
            Node b = ear;
            Node c = ear.next;
